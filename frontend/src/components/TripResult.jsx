...
const TripResult = ({ tripData, onClose }) => {
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      const W = 210; const MARGIN = 15; const CONTENT_W = W - MARGIN * 2;
      ...
      doc.text(tripData.destination, MARGIN, 80);

      doc.setFontSize(13); doc.setFont('helvetica', 'normal');
      doc.setTextColor(200, 200, 220);
      doc.text(`${tripData.duration}  |  ${tripData.dates}`, MARGIN, 95);
      ...
      const ovLines = doc.splitTextToSize(tripData.overview || '', CONTENT_W);
      doc.text(ovLines, MARGIN, 112);

      // Highlights box
      if (tripData.highlights?.length) {
        let hy = 112 + ovLines.length * 5 + 12;
        doc.setFillColor(30, 40, 70);
        doc.roundedRect(MARGIN, hy - 6, CONTENT_W, tripData.highlights.length * 8 + 14, 4, 4, 'F');
        doc.setFontSize(10); doc.setFont('helvetica', 'bold'); doc.setTextColor(244, 63, 94);
        doc.text('TOP HIGHLIGHTS', MARGIN + 5, hy + 2);
        hy += 10;
        doc.setFont('helvetica', 'normal'); doc.setTextColor(220, 225, 240);
        tripData.highlights.forEach(h => {
          const hLines = doc.splitTextToSize(`• ${h}`, CONTENT_W - 10);
          doc.text(hLines, MARGIN + 5, hy);
          hy += hLines.length * 6;
        });
      }

      // Footer on cover
      doc.setFontSize(8); doc.setTextColor(100, 110, 140);
      doc.text('Generated by AI Travel Planner  ✈', W / 2, 285, { align: 'center' });

      // ── PAGE 2: BUDGET ──
      let y = addPage();
      if (tripData.budgetEstimate) {
        y = sectionTitle(doc, '💰  BUDGET ESTIMATE', y);
        const entries = Object.entries(tripData.budgetEstimate);
        entries.forEach(([k, v], idx) => {
          y = checkY(y, 10);
          if (idx % 2 === 0) doc.setFillColor(245, 247, 255);
          else doc.setFillColor(255, 255, 255);
          doc.rect(MARGIN, y - 4, CONTENT_W, 9, 'F');
          doc.setFontSize(10); doc.setFont('helvetica', 'bold'); doc.setTextColor(60, 60, 80);
          doc.text(k.charAt(0).toUpperCase() + k.slice(1), MARGIN + 3, y + 2);
          doc.setFont('helvetica', 'normal'); doc.setTextColor(244, 63, 94);
          doc.text(String(v), W - MARGIN - 3, y + 2, { align: 'right' });
          y += 10;
        });
        y += 8;
      }
      ...
      if (tripData.travelTips?.length) {
        y = checkY(y, 20);
        y = sectionTitle(doc, '💡  TRAVEL TIPS', y);
        doc.setFontSize(9.5); doc.setFont('helvetica', 'normal'); doc.setTextColor(50, 50, 70);
        tripData.travelTips.forEach((t, i) => {
          y = checkY(y, 12);
      ...
      // Extra info
      const extras = [
        tripData.bestTimeToVisit && ['Best Time to Visit', tripData.bestTimeToVisit],
        tripData.localCurrency && ['Currency & Payments', tripData.localCurrency],
        tripData.language && ['Language', tripData.language],
        tripData.emergencyNumbers && ['Emergency Numbers', tripData.emergencyNumbers],
      ].filter(Boolean);

      if (extras.length) {
        y = checkY(y, 20);
        y = sectionTitle(doc, 'ℹ️  USEFUL INFORMATION', y);
        extras.forEach(([label, val]) => {
          ...
          const vLines = doc.splitTextToSize(String(val), CONTENT_W - 5);
          doc.text(vLines, MARGIN + 3, y + 5);
          y += vLines.length * 5 + 8;
        });
      }
      ...
      if (allDays.length) {
        allDays.forEach(day => {
          ...
          doc.text(`DAY ${day.day}  —  ${day.title}`, MARGIN, 10);
      ...
      for (let i = 1; i <= pages; i++) {
        ...
        doc.text(`AI Travel Planner  ·  ${tripData.destination}  ·  Page ${i} of ${pages}`, W / 2, 295, { align: 'center' });
      }
